...
class Move {
    ...
    boolean isJump() {
        if (Math.abs(col1() - col0()) == 2 || Math.abs(row0() - row1()) == 2) {
            return true;
        }
        return false;
    }
    ...
    /** Returns -Infinity value move. */
    static final Move LOW = new Move();
    /** Returns - Infinity value move. */
    static Move low() {
        return LOW;
    }
    /** @return  Checks to see if low move. */
    boolean isLow() {
        return this == LOW;
    }
    /** Returns Infinity value move. */
    static final Move HIGH = new Move();
    /** @return  Returns Infinity value move. */
    static Move high() {
        return HIGH;
    }
    /** @return  Checks to see if high move. */
    boolean isHigh() {
        return this == HIGH;
    }
