...
class Game {
        ...
        while (true) {
            ...
            while (_state != SETUP && !_board.gameOver()) {
                if (board().whoseMove().equals(RED)) {
                    if (redplayer.getClass().equals(Manual.class)) {
                        Command cmnd = getMoveCmnd(
                                board().whoseMove().toString() + ":");
                    ...
                    if (blueplayer.getClass().equals(Manual.class)) {
                        Command cmnd = getMoveCmnd(
                                _board.whoseMove().toString() + ":");
                        if (cmnd.commandType().equals(CLEAR)) {
                            doClear(cmnd.operands());
                            continue GameLoop;
                        } else {
    ...
    /** @param cmnd Performs a command. */
    void performCommand(Command cmnd) {
        if (cmnd.commandType().equals(QUIT)) {
            doQuit(cmnd.operands());
        } else if (cmnd.commandType().equals(DUMP)) {
            doDump(cmnd.operands());
        } else if (cmnd.commandType().equals(PIECEMOVE)) {
            doMove(cmnd.operands());
        } else if (cmnd.commandType().equals(BLOCK)) {
            System.out.println("Can only add blocks to initial configuration.");
    ...
    void doLoad(String[] operands) {
        try {
            ...
            CommandSource source = new ReaderSource(reader, false);
    ...
    void doMove(String[] operands) {
        ...
        if (!board().legalMove(Move.move(operands[0].charAt(0),
                operands[1].charAt(0),
    ...
    /** What type of player red is. */
    private Player redplayer = new Manual(this, RED);
    /** What type of player blue is. */
    private Player blueplayer = new AI(this, BLUE);
    /**For the case of you add a block,
     *  make a move (not start), and add another block. */
