...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Iterator<Integer> iterator = segment.iterator();
        int prev = iterator.next();
        int current = iterator.next();
        Road prevRoad = _map.getLabel(prev, current);
        String prevname = prevRoad.toString();
        String prevdirection = prevRoad.direction().fullName();
        double prevdistance = prevRoad.length();
        String currentName = "";
        String curdirection = "";
        String last = "";
        double distance = 0.0;
        while (iterator.hasNext()) {
            ...
            Road temp = _map.getLabel(prev, current);
            currentName = temp.toString();
            curdirection = temp.direction().fullName();
            distance = temp.length();
            if (currentName.equals(
                    prevname) && curdirection.equals(prevdirection)) {
                prevdistance += distance;
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.\n",
                        seq, prevname, prevdirection, prevdistance);
                prevname = currentName;
                prevdirection = curdirection;
                prevdistance = distance;
                seq += 1;
            }
            last = _map.getLabel(current).toString();
        }
        System.out.printf("%d. Take %s %s for %.1f miles to %s.\n",
                seq, currentName, curdirection, prevdistance, last);
        seq += 1;
        return seq;
    }
