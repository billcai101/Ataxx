...
class Rule {
    ...
    private void checkFinishedDependencies() {
        for (int successor : _depends.successors(getVertex())) {
            Rule rule = _depends.getLabel(successor);
            if (rule.isUnfinished()) {
                error("error dependencies unfinished");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int vertex : _depends.successors(getVertex())) {
            Rule rule = _depends.getLabel(vertex);
